...
;;;; Procedures
;; ex 4.16 seems better to put scan-out-defines into make-procedure 
;; since it avoids calling it every time the procedure is evaluated
;; (which would be the case if we put it into procedure-body). 
...
(define (scan-out-defines body)
  (define (let-binding-for define)
    (make-let-binding 
     (definition-variable define) 
     (make-quoted '*unassigned*)))
  (define (set-for define)
    (make-assignment 
     (definition-variable define) 
     (definition-value define)))
  (let ((defines (filter definition? body))
  	(not-defines (filter not-definition? body)))
    (if (null? defines)
	body
	(let ((bindings (map let-binding-for defines))
	      (sets (map set-for defines)))
	  (let ((new-body (append sets not-defines)))
