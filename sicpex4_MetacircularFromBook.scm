...
(define (eval exp env)
  ...
  (cond ((self-evaluating? exp) 
	 ...
	 (return-trace (lookup-variable-value exp env)))
        ((quoted? exp) 
	 (cdnl-trace "quoted") 
	 (return-trace (text-of-quotation exp)))
        ((assignment? exp) 
	 (cdnl-trace "assignment") 
	 (return-trace (eval-assignment exp env)))
        ((definition? exp) 
	 (cdnl-trace "definition") 
	 (return-trace (eval-definition exp env)))
        ((if? exp) 
	 (cdnl-trace "if") 
	 (return-trace (eval-if exp env)))
        ((lambda? exp)
	 (cdnl-trace "lambda") 
	 (return-trace 
	 ...
	 (return-trace 
	  (eval-sequence (begin-actions exp) env)))
        ((cond? exp) 
	 (cdnl-trace "cond") 
	 (return-trace (eval (cond->if exp) env)))
        ((application? exp)
...
;; Eval: provedure arguments, if, sequence, assignment, definition
